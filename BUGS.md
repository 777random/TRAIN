...
| B25 | Coach zeigt "Auf Kurs" trotz 4 Wochen komplettem Fail | **UX-Hoch** | Gefunden bei Edge-Case-Audit v159 (TRAIN_Test_EdgeCase_AllesFail_GuterSchlaf.v1.json — AllesFail-Fixture ohne Schlaf-Störfaktor, RPE 9.5-10 durchgehend, Wdh sinken Woche für Woche). Root Cause: JEDE Funktion in der computeWeeklyFocus()-Kaskade (weeklyFocus.js) filtert auf `status === 'success'`, bevor sie irgendein Signal berechnet — `_checkRisingRpe()` (Zeile 166-191) bricht bei Zeile 178 (`weights.some(w => w === 0)`) sofort ab, weil ohne Success-Sätze `max` nie über 0 kommt, die RPE-Prüfung (Zeile 185-187) wird nie erreicht. `_checkPlateau()` (detectPlateaus, nutzt isFullSuccess) und `_checkProgression()` (Zeile 575: `calcWeeks` filtert explizit auf `s.status === 'success'`) fallen aus demselben Grund aus. `_checkConsistencyGap()` misst Anwesenheit (by design, DECISIONS.md), nicht Erfolg — greift hier korrekt nicht. Ergebnis: Bei einem Nutzer, der wochenlang jeden Satz verfehlt, aber brav weitertrainiert, sagt die App "Keine besonderen Auffälligkeiten — trainiere wie geplant weiter." Das widerspricht der Kernmission ("Decision Support"). Braucht eine Produktentscheidung (neues Signal? welche Schwelle? welche Formulierung?) — nicht als Quick-Fix behandeln, siehe CLAUDE.md "zwei Pflichtfragen vor konzeptionell neuen Features". Noch NICHT gefixt. |
