...
### 2026-07 — Reduzierte Tagesform dämpft nur Halten/Reduzieren, nie eine echte Steigerung (B84)
**Entscheidung:** `_applyModifier()` (sessionCoach.js) wendet die "reduzierte Tagesform"-Dämpfung (`sessionModifier==='reduced'`, B76) nur noch an, wenn die zugrundeliegende Empfehlung selbst ein Halten oder eine Reduktion ist (`nextWeight <= currentWeight`). Eine echte Steigerungs-Empfehlung (RPE≤6, `nextWeight = currentWeight + step`) wird NIE gedämpft — sie bleibt exakt `currentWeight + step`, unabhängig vom Tagesform-Modifier.
**Begründung:** Die ursprüngliche Formel (`Math.max(nextWeight * 0.9, currentWeight - step)`) wurde für den Fall entworfen, dass eine Reduktion an einem schlechten Tag nicht zu stark ausfällt — sinnvoll, solange `nextWeight <= currentWeight` gilt. Bei einer Steigerung (RPE war trotz reduziertem Tagesstart niedrig — ein positives Signal) erzeugt dieselbe Formel eine Zahl UNTER dem gerade gehobenen Gewicht (B84: 98kg RPE6 zeigte "95kg" statt einer Steigerung auf ~100kg), was dem gleichzeitig angezeigten "Noch Luft — steigern"-Text direkt widerspricht. Ein niedriges RPE ist ein positives Signal, das nicht durch die "reduzierte Tagesform"-Einstufung des GESAMTEN Tages entwertet werden sollte — wenn der Athlet trotz reduziertem Start mühelos mehr schafft, soll die Empfehlung das auch zeigen.
**Gilt:** Permanent bis gegenteilige Entscheidung. Neue Tagesform-Dämpfungslogik (falls je erweitert) prüft immer zuerst, ob die zugrundeliegende Basis-Empfehlung eine Steigerung oder ein Halten/Reduzieren ist, und dämpft nur Letzteres.
