...
### 2026-07 — B99: Wochenbezeichnung (Header) und Wochenrückblick-Dropdown zu einer Funktion konsolidiert, obwohl nur eine Seite den Bug hatte
**Entscheidung:** `_relativeWeekLabel()` (Wochen-Navigations-Header, verglich fehlerhaft gegen `getLatestWeek(weeks)`) und `_relDate()` (Wochenrückblick-Dropdown im Fortschritt-Tab, bereits korrekt gegen echtes `new Date()`) werden zu einer gemeinsamen `_weekLabel(week, weeks)` + `_calendarCurrentWeek(weeks)` zusammengeführt, obwohl `_relDate()` selbst keinen Bug hatte.
**Begründung:** Nutzer-Anfrage ging davon aus, das Dropdown habe "denselben Bug" — Diagnose widerlegte das (Dropdown listet ohnehin nur `markedDone`-Tage, eine frische Zukunftswoche taucht dort nie auf). Per Rückfrage (`AskUserQuestion`) trotzdem konsolidiert: zwei separate Implementierungen derselben Grundidee ("wie nennen wir eine Woche relativ zu heute") widersprechen der bestehenden DRY-Präzedenz (B44/B45/B74, "eine Formel, keine Kopien") — auch wenn nur eine Kopie fehlerhaft war, hätte ein künftiger Fix sonst wieder an nur einer der beiden Stellen angewendet werden können.
**Label-Schema bewusst verengt:** die neue `_weekLabel()` kennt nur noch "Diese Woche"/"Nächste Woche"/"Letzte Woche" + direkten "KW N · Jahr"-Fallback. Die bisherigen Zwischenstufen der alten `_relativeWeekLabel()` ("Vorletzte Woche"/"Vor N Wochen"/"In N Wochen") entfallen ersatzlos — vom Nutzer explizit bestätigt ("Ja, wie in der Vorlage"), keine Rückwärtskompatibilität zur alten Abstufung gewünscht.
**Technisches Detail:** lokale Datumsarithmetik (`new Date()` + `setHours(0,0,0,0)`, kein `toISOString()`) statt der in der Sprint-Vorlage vorgeschlagenen UTC-Rundung — Zeitzonen-Rollover-Schutz konsistent mit dem bereits etablierten Muster im Rest der Codebasis (`_relDate()`-Vorgänger, `_dayDate()`, `wkRange()`). `_calendarCurrentWeek()` liefert `null`, wenn keine Woche im State das heutige Datum enthält — `_weekLabel()` fällt dann sofort auf den KW-Fallback zurück, statt (wie die Vorlage es implizit erlaubt hätte) noch einen Next/Last-Monday-Vergleich gegen ein nicht-existentes Referenzdatum zu versuchen.
**Bewusst nicht angefasst:** `_isTodayDay()` (Session-Coach-Gate) teilt exakt dasselbe `getLatestWeek()`-als-"heute"-Muster, das hier gefixt wurde — bleibt laut B82 unverändert, da dort bewusst "zuletzt aktive Trainingswoche" statt "Kalenderdatum" gilt (Mo/Mi/Fr-Splits).
**Gilt:** Permanent bis gegenteilige Entscheidung. Neue relative Wochen-/Datumsbezeichnungen werden ausschließlich über `_weekLabel()` gebaut, keine dritte Parallel-Implementierung.
**Revidiert durch B120 (2026-07-26, train-v217):** Die "ersatzlos entfallen"-Verengung wurde zurückgenommen — `_weekLabel()` bekam die Zwischenstufe "Vor N Wochen" (2-8 Wochen zurück) zurück, nach expliziter Rückfrage bei genau diesem Zielkonflikt. KW-Fallback gilt weiterhin ab 9 Wochen zurück sowie für alle Zukunftswochen jenseits "Nächste Woche".

### 2026-07 — B100: reale Zeitstempel (sessionEndTs/sessionStartTs) statt Array-Index-Schätzung für "Letzte Einheit"-Datumsangaben
**Entscheidung:** `_trainingContextAnchor()` (ui.js) berechnet das Datum des zuletzt aktiven Tages jetzt über `_realDayDate()` — bevorzugt `day.sessionEndTs` (Tagesabschluss), dann `day.sessionStartTs` (Session begonnen, noch offen), erst danach die alte `_dayDate()`-Index-Schätzung (`startDate + dayIndex`) als Fallback für Alt-Daten ganz ohne Zeitstempel.
**Begründung:** Die Fix-Vorlage nahm eine nicht existierende Funktion `_daysSince()` an und schlug als "Fix" exakt dieselbe Formel vor, die bereits den gemeldeten Bug verursachte (`startDate + dayIndex`). Vor der Umsetzung geprüft: diese Formel kann die eigene AC1-5 (dichte, tägliche Woche) und AC6 (Mo/Mi/Fr-Split) nicht gleichzeitig erfüllen — eine reine Funktion von `dayIndex` kann nicht wissen, welcher reale Wochentag ein Array-Slot ist, ohne eine zusätzliche Datenquelle. Nach Rückfrage (`AskUserQuestion`) wurde eine Scope-Erweiterung auf state.js (neues Feld) bestätigt, bevor implementiert wurde.
**Positiver Fund beim Umsetzen:** kein neues Feld nötig. `day.sessionEndTs` existiert bereits seit SCHEMA 12 (`DAY_TOGGLE_COMPLETE`-Reducer, state.js) und wird bei jedem Tagesabschluss auf den echten `Date.now()`-Zeitstempel gesetzt, unabhängig vom Array-Index — der Fix bleibt dadurch entgegen der Rückfrage-Antwort doch bei "Nur ui.js", state.js unverändert.
**Bewusst nicht angefasst:** `_weekdayName()` (progressInsights.js, "erfolgreichster Wochentag"-Insight) teilt exakt dieselbe Index-als-Kalendertag-Annahme, die hier gefixt wurde — nicht Teil dieses Fixes, da nicht gemeldet; potenzieller Folge-Fix mit demselben Muster (`day.sessionEndTs` bevorzugen), falls das später relevant wird.
**Gilt:** Permanent bis gegenteilige Entscheidung. Ein "Fix"-Vorschlag, der exakt dieselbe Formel wie der diagnostizierte Bug verwendet, wird vor der Umsetzung gegen die eigenen Akzeptanzkriterien geprüft, nicht blind übernommen.

### 2026-07 — B101: Rundung auf ex.weightStep zentral in _applyPlannedProgression(), nicht an jeder Delta-Berechnungsstelle
**Entscheidung:** `_applyPlannedProgression()` (state.js) rundet den auf ein Set angewendeten Wert jetzt auf `ex.weightStep` (`Math.round(raw / step) * step`), direkt an der einen Stelle, an der jeder mögliche Weg zu `ex.nextWeekPlan` zusammenläuft — statt die Rundung an jeder einzelnen Delta-Berechnungsstelle (Recovery-Boost, Custom-Wert-Eingabe, künftige Wege) separat zu erzwingen. Funktion bekommt dafür `state` als zweiten Parameter (für den `plateStep`-Fallback, falls `ex.weightStep` fehlt).
**Begründung:** Die Fix-Vorlage nahm an, `_applyPlannedProgression()` verwende `state.settings.plateStep` statt `ex.weightStep` — das stimmte nicht, diese Funktion referenziert `plateStep` gar nicht, sie wendet nur einen bereits andernorts berechneten, bereits bestätigten Delta an. Der tatsächliche Bug: keine Rundung nach der Addition. Der Recovery-Boost (`ui.js`, `rec.delta *= 1.5` bei `isInRecoveryWindow()`) rundet nur den Anzeigewert (`recommendedWeight`) neu, nicht `rec.delta` selbst — das ungerundete Delta wird unverändert als `ex.nextWeekPlan` gespeichert und ungerundet angewendet, wodurch das im "Neue Woche"-Modal versprochene Gewicht vom tatsächlich gesetzten abweicht.
**Zwei Root Causes der Vorlage entfielen nach Prüfung (per Rückfrage bestätigt):** (1) "plateStep statt weightStep in `_applyPlannedProgression()`" — existiert nicht, siehe oben; die zwei echten `plateStep`-ohne-Fallback-Stellen liegen in einem anderen Feature (`weeklyFocus.js` `_checkPersistentFailure`/`_checkMultiExerciseFailure`, Gewichts-Reduktionsvorschlag bei Dauer-Fehlschlägen), dort trotzdem mitgefixt. (2) "null-Fallback ignoriert weightStep" — kein Code dafür vorhanden; ohne ≥2 Wochen Historie wird bewusst gar keine Empfehlung berechnet (`getWeightRecommendation()` liefert `null`), kein Fallback-Wert existiert der gerundet werden müsste. Nach Rückfrage bewusst NICHT neu eingeführt — wäre eine neue Funktion, kein Bugfix.
**Gilt:** Permanent bis gegenteilige Entscheidung. Rundungslogik für gestufte Gewichte (weightStep) wird am Anwendungspunkt zentralisiert, nicht an jeder Stelle wiederholt, die einen Delta-Wert erzeugt.

### 2026-07 — B127: Zwei unabhängige Notiz-Arten pro Übung statt einer
**Entscheidung:** `ex.note` (tagesspezifisch — Befinden, Beobachtungen zur heutigen Einheit) bekommt eine neue, bisher nicht existierende Live-Editier-UI im Trainings-Tab (vorher nur über den Vorlagen-Editor erreichbar) und wird jetzt konsequent bei jedem Klon-/Speicher-Vorgang zurückgesetzt (`_resetExerciseSubstitution()`, state.js — dieselbe Funktion, die bereits B114s Substitutions-Reset übernimmt). Neues `state.exerciseNotes: { [exerciseName]: string }` (permanent — Rack-Höhe, Griffbreite, Technik-Cues) übersteht Wochen-Klone bewusst. `s.note` (pro Satz, bestehend) bleibt ein drittes, unverändertes, unabhängiges Feld — keine Zusammenlegung der drei Konzepte.
**Befund vor der Umsetzung:** die Sprint-Vorlage nahm an, `ex.note` sei bereits das live editierbare "Heute"-Feld im Tages-View — tatsächlich hatte es dort gar keine Editier-UI (nur Vorlagen-Editor), und überlebte Wochen-Klone bisher nur zufällig (in keinem Klon-Reducer erwähnt, nicht bewusst so entworfen). Die neue Live-UI und der Reset-Zusatz sind damit Teil dieses Fixes, nicht nur die neue `exerciseNotes`-Ergänzung.
